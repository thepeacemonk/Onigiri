...
(function () {
    ...
    function setupResetButton() {
        ...
        const toolbar = setupSidebarToolbar();
        if (!toolbar || toolbar.querySelector('.deck-reset-btn')) return;

        const resetBtn = document.createElement('div');
        resetBtn.className = 'deck-reset-btn';
        resetBtn.title = 'Reset/Reload Dashboard';
        resetBtn.innerHTML = `<i class="icon"></i>`;
        ...
        resetBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            if (typeof pycmd === 'function') {
                pycmd('onigiri_reload_dashboard');
            }
        });
    }
