...
class LightDarkModeSwitch(QAbstractButton):
    """Self-contained light/dark pill switch.

    One control: the sliding knob carries the active icon (sun when light, moon
    when dark) while the opposite icon sits faint on the track. Clicking anywhere
    flips the mode. Follows the active accent color.
    """
    def __init__(self, parent=None, accent_color="#008cff"):
        super().__init__(parent)
        self.setCheckable(True)
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.accent_color = QColor(accent_color)
        self.track_color_off = QColor("#cbd0d8") if not theme_manager.night_mode else QColor("#3f4652")
    ...
    def _thumb_radius(self):
        return (self.height() - 2 * self._margin()) / 2.0
    ...
    def _checked_thumb_position(self, checked=None):
        ...
        m = self._margin()
        if checked:
            return float(self.width() - m - self._thumb_radius() * 2)
        return float(m)
    ...
    def setThemeMode(self, is_dark):
        self.track_color_off = QColor("#3f4652") if is_dark else QColor("#cbd0d8")
        self.update()
    ...
    def _start_animation(self, checked):
        self.animation.stop()
        self.animation.setStartValue(self.thumb_x_pos)
        self.animation.setEndValue(self._checked_thumb_position(checked))
        self.animation.start()

    # --- icons ----------------------------------------------------------
    def _icon_pixmap(self, filename, color, px):
        key = (filename, color.rgba(), px, round(self.devicePixelRatioF() * 100))
        cached = self._icon_cache.get(key)
        if cached is not None:
            return cached
        path = system_icon_path(filename)
        if not path or not os.path.exists(path):
            return QPixmap()
        dpr = max(1.0, self.devicePixelRatioF())
        render_px = max(1, int(round(px * dpr)))
        pixmap = QIcon(path).pixmap(render_px, render_px)
        if pixmap.isNull():
            return pixmap
    ...
    def _draw_icon(self, painter, filename, center, px, color):
        pixmap = self._icon_pixmap(filename, color, px)
        if pixmap.isNull():
            return
        dpr = pixmap.devicePixelRatio()
        w = pixmap.width() / dpr
        h = pixmap.height() / dpr
        painter.drawPixmap(QPointF(center.x() - w / 2.0, center.y() - h / 2.0), pixmap)
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(Qt.PenStyle.NoPen)
        # tie the visuals to the knob's live position so icons arrive with the slide
        left = self._checked_thumb_position(False)
        right = self._checked_thumb_position(True)
        progress = 0.0 if right == left else max(0.0, min(1.0, (self._thumb_x_pos - left) / (right - left)))
        on_dark = progress >= 0.5

        if not enabled:
            # same track grey as the enabled state so the switch reads identically
            # on every settings page; only the knob icon dims to signal disabled
            track_color = self.track_color_off
        else:
            # blend track between off-color (light) and accent (dark) as it slides
            track_color = self._blend(self.track_color_off, self.accent_color, progress)
        painter.setBrush(track_color)
        painter.drawRoundedRect(track, radius, radius)

        thumb_r = self._thumb_radius()
        cy = self.height() / 2.0
        knob_icon_px = int(round(thumb_r * 0.55))   # small icon inside the knob
        track_icon_px = int(round(thumb_r * 0.5))   # faint icon on the track
        thumb_center = QPointF(self._thumb_x_pos + thumb_r, cy)
        left_center = QPointF(self._margin() + thumb_r, cy)
        right_center = QPointF(self.width() - self._margin() - thumb_r, cy)

        # inactive icon: faint, on the opposite side of the track
        faint = QColor(255, 255, 255, 210)
        if on_dark:
            self._draw_icon(painter, "sun.svg", left_center, track_icon_px, faint)
        else:
            self._draw_icon(painter, "moon.svg", right_center, track_icon_px, faint)
        ...
        # active icon inside the knob, in the accent color
        active_color = QColor("#9aa1ac") if not enabled else self.accent_color
        if on_dark:
            self._draw_icon(painter, "moon.svg", thumb_center, knob_icon_px, active_color)
        else:
            self._draw_icon(painter, "sun.svg", thumb_center, knob_icon_px, active_color)
...
class GooeySegmentButton(QPushButton):
    """Text (or glyph) segment; the parent shell paints the animated blob.

    `icon_painter` is an optional `(painter, rect, color) -> None` callback. When
    given it draws the segment instead of the label, so a control can offer a
    shape glyph where a word would be clumsy; the text is still set, so it stays
    available as the accessible name and the tooltip.
    """
    def __init__(self, text, fg, checked_fg="#ffffff", hover_fg=None, parent=None, icon_painter=None):
        super().__init__(text, parent)
        self._fg = QColor(fg)
        self._checked_fg = QColor(checked_fg)
        self._hover_fg = QColor(hover_fg or fg)
        self._icon_painter = icon_painter
        self.setCheckable(True)
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.setFlat(True)
    ...
    def paintEvent(self, event):
        ...
        if self.isChecked():
            ...
            color = self._fg
        if self._icon_painter is not None:
            painter.setRenderHint(QPainter.RenderHint.Antialiasing, True)
            self._icon_painter(painter, QRectF(self.rect()), color)
            painter.end()
            return
...
class GooeySegmentShell(QFrame):
    ...
    def clear_blob(self):
        """Hide the selection blob entirely (no segment active)."""
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # No checked segment -> no blob (the value lives in a companion spin box).
        checked = None
...
class ProfileBarWidget(QWidget):
    ...
    def set_name_font_override(self, font_key):
        self._name_font_override = font_key or "system"
        self.refresh_theme()

    def _name_font_family(self):
        from ._common import mw
        font_key = self._name_font_override
        if font_key is None:
            font_key = mw.col.conf.get("modern_menu_profile_name_font", "system")
        if not font_key or font_key == "system":
            return ""
        cache = globals().setdefault("_PROFILE_NAME_FONT_CACHE", {})
        if font_key in cache:
            return cache[font_key]
        family = ""
        try:
            info = get_all_fonts(ADDON_ROOT).get(font_key, {})
            font_file = info.get("file")
            if font_file:
                if info.get("user"):
                    path = os.path.join(ADDON_ROOT, "user_files", "fonts", font_file)
                else:
                    path = os.path.join(ADDON_ROOT, "system_files", "fonts", "system_fonts", font_file)
    ...
    def refresh_theme(self):
        ...
        family = self._name_font_family()
        family_css = f"font-family: '{family}';" if family else ""
        self.name_label.setStyleSheet(
            f"font-weight: 500; font-size: 16px; color: {name_color}; background: transparent; {family_css}"
        )
        self.update()
