...
class InfraMixin:
    ...
    def _populate_page_nav(self, page_name):
        ...
        # Page titles were removed, so the shared header only exists to hold the
        # section nav buttons (and, on Themes, the light/dark toggle). Hide the
        # whole header band when there's nothing to show, instead of leaving an
        # empty gap above the page. Custom-header pages manage this themselves.
        if hasattr(self, "settings_page_header"):
            has_custom_header = page_name in {"Sync", "Fonts", "Gallery"}
            has_action = page_name == "Themes"
            if not has_custom_header:
                self.settings_page_header.setVisible(bool(sections) or has_action)
    ...
    def _action_add_dashed_enabled(self):
        toggle = getattr(self, "action_add_dashed_toggle", None)
        if toggle is not None:
            return toggle.isChecked()
        return bool(self.current_config.get("sidebarAddDashed", False))

    def _on_action_add_dashed_toggled(self, checked):
        self._update_action_buttons_preview()
    ...
    def _action_button_icon_size_ratio(self):
        widget = getattr(self, "icon_size_widgets", {}).get("action_button")
        value = widget.value() if widget else mw.col.conf.get(
            "modern_menu_icon_size_action_button", DEFAULT_ICON_SIZES["action_button"]
        )
        return value / DEFAULT_ICON_SIZES["action_button"]
    ...
    def _measure_action_buttons_list(self, ids, card_width):
        ...
        total = 0.0
        add_height = row_height * 1.25 if self._action_add_dashed_enabled() else row_height
        for action_id in ids:
            if action_id == "add":
                total += add_height + gap
            elif action_id == "more":
    ...
    def _draw_action_buttons_list(self, painter, rect, mode, ids):
        ...
        sub_row = row_height * 0.9
        icon_size = row_height * 0.56 * self._action_button_icon_size_ratio()
        icon_size = max(row_height * 0.22, min(icon_size, row_height * 0.92))
        ...
        for action_id in ids:
            if draw_row.y > rect.bottom() + 0.5:
                break
            if action_id == "add":
                dashed = self._action_add_dashed_enabled()
                height = row_height * 1.25 if dashed else row_height
                draw_row(action_id, 0.0, height, dashed=dashed)
    ...
    def _draw_action_buttons_collapsed(self, painter, rect, mode, ids):
        ...
        for index, action_id in enumerate(ids):
            ...
            icon_frac = max(0.20, min(0.52 * self._action_button_icon_size_ratio(), 0.88))
            icon_pad = button_size * (1 - icon_frac) / 2
            icon_rect = button_rect.adjusted(icon_pad, icon_pad, -icon_pad, -icon_pad)
    ...
    def _create_action_buttons_designer(self):
        ...
        action_button_icon_size_spinbox = self.create_icon_size_spinbox("action_button", DEFAULT_ICON_SIZES["action_button"])
        action_button_icon_size_spinbox.setMinimumWidth(120)
        action_button_icon_size_spinbox.valueChanged.connect(self._update_action_buttons_preview)
        controls_layout.addWidget(
            self._create_main_bg_value_row(tr("action_button_icons"), action_button_icon_size_spinbox)
        )

        self.action_add_dashed_toggle = AnimatedToggleButton(accent_color=self.accent_color)
        self.action_add_dashed_toggle.setChecked(bool(self.current_config.get("sidebarAddDashed", False)))
        self.action_add_dashed_toggle.toggled.connect(self._on_action_add_dashed_toggled)
        controls_layout.addWidget(
            self._create_main_bg_toggle_row(tr("add_button_dashed_label", "Dashed outline on Add"), self.action_add_dashed_toggle)
        )
    ...
    def _reset_action_buttons_to_default(self):
        ...
        toggle = getattr(self, "action_add_dashed_toggle", None)
        if toggle is not None:
            toggle.setChecked(bool(DEFAULTS.get("sidebarAddDashed", False)))
    ...
    def _create_box_effect_retention_star_section(self):
        ...
        # Retention stars used to live here. They belong to the Retention stat
        # card, so the toggle, the star icon and the star colors now sit in the
        # Stats Widgets section (see _page_stats_widgets.py). These cards stay
        # hidden: they are the line edits the star colors are persisted from.
        for card in (
    ...
    def _on_box_effect_changed(self, *args):
        ...
        if hasattr(self, "sidebar_bg_sync_box_toggle") and self.sidebar_bg_sync_box_toggle.isChecked():
            self._update_modern_background_preview("sidebar")
        if hasattr(self, "deck_stats_sync_toggle") and self.deck_stats_sync_toggle.isChecked():
    ...
    def _draw_box_effect_sample(self, painter, rect, mode, background_pixmap):
        ...
        hide_stars = self._retention_stars_hidden()
...
class InfraMixin2:
    def _create_deck_icon_designer_group(self):
        ...
        deck_icon_setting_rows.append(create_setting_row(tr("hide_folder_icon"), self.hide_folder_cb))
        ...
        deck_icon_setting_rows.append(create_setting_row(tr("hide_subdeck_icon"), self.hide_subdeck_cb))
        ...
        deck_icon_setting_rows.append(create_setting_row(tr("hide_deck_icon"), self.hide_deck_cb))
        ...
        deck_icon_setting_rows.append(create_setting_row(tr("hide_filtered_deck_icon"), self.hide_filtered_deck_cb))
        ...
        deck_icon_setting_rows.append(create_setting_row(tr("hide_default_show_custom"), self.hide_default_custom_cb))

        self.hide_deck_counts_checkbox.toggled.connect(self._update_deck_icon_preview)
        deck_icon_setting_rows.append(create_setting_row(tr("hide_zero_counts"), self.hide_deck_counts_checkbox))

        self.hide_all_deck_counts_checkbox.toggled.connect(self._update_deck_icon_preview)
        deck_icon_setting_rows.append(create_setting_row(tr("hide_all_counts"), self.hide_all_deck_counts_checkbox))

        icon_sizes_to_configure = {"deck_folder": tr("deck_folder_icons_label"), "collapse": tr("expand_collapse_icons_label")}
        for key, label in icon_sizes_to_configure.items():
            ...
            deck_icon_setting_rows.append(create_setting_row(label, size_spinbox))

        deck_icon_settings_grid = QGridLayout()
        deck_icon_settings_grid.setHorizontalSpacing(8)
        deck_icon_settings_grid.setVerticalSpacing(8)
        deck_icon_settings_grid.setColumnStretch(0, 1)
        deck_icon_settings_grid.setColumnStretch(1, 1)
        for index, row in enumerate(deck_icon_setting_rows):
            deck_icon_settings_grid.addWidget(row, index // 2, index % 2)
        sizing_layout.addLayout(deck_icon_settings_grid)
    ...
    def eventFilter(self, source, event):
        ...
        try:
            ...
            if source.property("deck_stats_preview") and event.type() == QEvent.Type.Resize:
                QTimer.singleShot(0, self._update_deck_stats_preview)
                return False

            if source.property("stats_widgets_preview") and event.type() == QEvent.Type.Resize:
                QTimer.singleShot(0, self._update_stats_widgets_preview)
                return False

            if source.property("hashi_widget_preview") and event.type() == QEvent.Type.Resize:
                QTimer.singleShot(0, self._update_hashi_widget_preview)
                return False
    ...
    def _create_light_dark_mode_toggle(self, current_mode, on_change):
        ...
        widget.setStyleSheet(
            "QWidget#lightDarkModeToggle { background: transparent; }"
        )
        ...
        toggle = LightDarkModeSwitch(self, accent)
        toggle.setChecked(current_mode == "dark")
        toggle.setToolTip(tr("light_mode", "Light") + " / " + tr("dark_mode", "Dark"))
