...
class PageReviewerMixin:
    ...
    def _reviewer_stattxt_mode(self):
        ...
        if mode in {"hover", "inverted", "fixed", "off"}:
            return mode
    ...
    def _set_reviewer_stattxt_mode(self, mode):
        ...
        if not group:
            return
        if mode not in {"hover", "inverted", "fixed", "off"}:
            mode = "hover"
    ...
    def _reviewer_timer_position(self):
        ...
        if position in {"right", "left", "out", "off"}:
            return position
    ...
    def _set_reviewer_timer_position(self, position):
        ...
        if not group:
            return
        if position not in {"right", "left", "out", "off"}:
            position = "right"
        for button in group.buttons():
            with QSignalBlocker(button):
    ...
    def _on_reviewer_timer_position_changed(self):
        if hasattr(self, "reviewer_timer_color_section"):
            # "out" styles the timer like the other bottom-bar buttons and "off"
            # hides it entirely, so the timer-pill colors don't apply to either.
            locked = self._reviewer_timer_position() in {"out", "off"}
    ...
    def create_bottom_bar_background_and_buttons_section(self):
        ...
        self._reviewer_sticky_panel_bg = _sticky_panel_bg
        ...
        if current_stattxt_mode not in {"hover", "inverted", "fixed", "off"}:
        ...
        stattxt_mode_container = self._create_organize_segmented_control(
            [
                ...
                ("inverted", tr("stats_numbers_inverted_short", "Inverted")),
        ...
        if current_timer_position not in {"right", "left", "out", "off"}:
        ...
        timer_position_container = self._create_organize_segmented_control(
            [
                ...
                ("off", tr("timer_position_off_short", "Off")),
            ],
    ...
    def _reposition_reviewer_sticky(self):
        ...
        try:
            ...
            if not getattr(self, "_reviewer_sticky_installed", False):
                ...
                # corners (the QSS rounded background alone leaves those corners
                # unpainted).
                shade = QWidget(viewport)
                shade.setObjectName("reviewerStickyShade")
                shade.setStyleSheet(
                    f"QWidget#reviewerStickyShade {{ background-color: {self._reviewer_sticky_panel_bg}; border: none; }}"
                )
                shade.show()
                self._reviewer_sticky_shade = shade
            ...
            shade = getattr(self, "_reviewer_sticky_shade", None)
    ...
    def _draw_bottom_bar_preview_button(self, painter, rect, label, key, theme_mode, hovered=False, pre_answer_counts=False):
        if self.reviewer_btn_custom_enable_toggle.isChecked():
            ...
            if pre_answer_counts:
                ...
                # "fixed" always shows the stats bar; "hover" only on hover;
                # "inverted" shows the counts (and thus the stats bar) at rest.
                if (
                    (preview_stattxt_mode == "hover" and hovered)
                    or preview_stattxt_mode == "fixed"
                    or preview_stattxt_mode == "inverted"
                ):
        ...
        stat_samples = {"again": "<1m", "hard": "<5m", "good": "<10m", "easy": "5d"}
        ...
        elif stattxt_mode == "inverted" and pre_answer_counts and not hovered:
            # Inverted: counts sit on the Show Answer button at rest; the
            # "Show Answer" label only appears on hover (handled by the else).
            self._draw_bottom_bar_pre_answer_counts(painter, rect, theme_mode)
        ...
        elif stattxt_mode == "inverted" and is_answer_key and not hovered:
            # Inverted: the interval number shows at rest; the ease label only
            # appears on hover (handled by the else branch).
            stat_font = QFont(font)
            stat_font.setBold(True)
            stat_font.setPointSize(max(9, min(16, int(rect.height() * 0.38))))
            painter.setFont(stat_font)
    ...
    def _update_reviewer_bottom_bar_preview(self):
        ...
        if preview_mode == "pre_answer":
            ...
            # Answer state: Edit is pinned to the far left and More to the far
            # right (mirroring the pre-answer preview); the ease buttons stay
            # centered. The after-flip timer rides just inside Edit or More per
            # the reviewer rules: Right -> left of More, Left -> right of Edit,
            # Out -> left of More. It is never placed among the ease buttons.
            timer_pos = self._reviewer_timer_position()
            ease_specs = [
            ...
            left_specs = [("Edit", "other", 64, "edit")]
            right_specs = [("More", "other", 64, "more")]
            timer_spec = ("0:06", "other", 64, "timer_out")
            if timer_pos == "left":
                left_specs.append(timer_spec)
            elif timer_pos in ("right", "out"):
                right_specs.insert(0, timer_spec)

            side_margin = max(18, width * 0.04)
            gap = 10
            all_specs = left_specs + ease_specs + right_specs
            total = sum(item[2] for item in all_specs) + gap * (len(all_specs) - 1)
            scale = min(1.0, max(0.55, (width - side_margin * 2) / max(1, total)))
            button_gap = gap * scale

            def _answer_button_w(base_w):
                return max(46, base_w * scale + padding)

            def _draw_answer_spec(label, key, base_w, button_id, x):
                button_w = _answer_button_w(base_w)
                rect = QRectF(x, y, button_w, button_h)
                hovered = hover_key == button_id
                self._bottom_bar_preview_button_rects.append((button_id, QRectF(rect)))
                self._draw_bottom_bar_preview_button(painter, rect, label, key, mode, hovered=hovered)
                return button_w

            # Left-pinned group (Edit, plus timer when "left").
            x = side_margin
            for label, key, base_w, button_id in left_specs:
                x += _draw_answer_spec(label, key, base_w, button_id, x) + button_gap

            # Right-pinned group (timer when "right"/"out", plus More), laid out
            # so it ends flush at width - side_margin.
            right_widths = [_answer_button_w(item[2]) for item in right_specs]
            right_total = sum(right_widths) + button_gap * (len(right_specs) - 1)
            x = width - side_margin - right_total
            for label, key, base_w, button_id in right_specs:
                x += _draw_answer_spec(label, key, base_w, button_id, x) + button_gap

            # Centered ease-button group.
            ease_widths = [_answer_button_w(item[2]) for item in ease_specs]
            ease_total = sum(ease_widths) + button_gap * (len(ease_specs) - 1)
            x = (width - ease_total) / 2
            for label, key, base_w, button_id in ease_specs:
                x += _draw_answer_spec(label, key, base_w, button_id, x) + button_gap
        painter.end()
