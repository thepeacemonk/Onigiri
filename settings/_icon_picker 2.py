...
class LongPressIconCell(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if self.progress > 0:
            ...
            clip_path.addRoundedRect(0, 0, self.width(), self.height(), CELL_RADIUS, CELL_RADIUS)
...
class IconPickerDialog(QDialog):
    ...
    def __init__(self, current_filename, addon_path, parent=None):
        ...
        self._pal = picker_palette(bool(theme_manager.night_mode))
        ...
        title_label.setStyleSheet(title_qss(self._pal))
        ...
        for btn in (add_btn, default_btn):
        ...
        scroll_area.setStyleSheet(scroll_qss(self._pal))

        self.grid_content = QWidget()
        self.grid_layout = FlowLayout(self.grid_content, margin=0, spacing=8)
    ...
    def _load_icons(self):
        ...
        for filename in files:
            container = QFrame()
            container.setFixedSize(CELL_SIZE, CELL_SIZE)
            container.setObjectName("IconItem")
            container.setStyleSheet(cell_qss(self._pal, filename == self.current_filename))
            ...
            del_btn.move(CELL_SIZE - 22, 4) # Position top-right

            # Load xmark icon
            xmark_path = system_icon_path("xmark.svg")

            # Style for delete button: No background, no hover effect as requested
            del_btn.setStyleSheet("QPushButton { background-color: transparent; border: none; }")
            icon_color = self._pal["muted"]
    ...
    def _filter_icons(self, text):
        ...
        for filename in filtered_files:
            ...
            layout = QVBoxLayout(container); layout.setContentsMargins(6, 6, 6, 6)
            ...
            btn = QPushButton(container)
            btn.setStyleSheet("background: transparent; border: none;")
            btn.setFixedSize(CELL_SIZE, CELL_SIZE); btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.clicked.connect(lambda _, f=filename: self._select_icon(f))
            btn.move(0, 0)
...
class DeckIconPickerDialog(QDialog):
    ...
    def __init__(self, current_icon, addon_path, parent=None, allow_emoji=True, color_options=None, preview_color_key=None, night_mode=None):
        ...
        self.tabs.setStyleSheet(tabs_qss(self._pal))
    ...
    def _on_preview_mode_toggled(self, mode):
        ...
        if self.color_options and hasattr(self, "colors_panel_widget"):
            self.colors_panel_widget.setStyleSheet(panel_qss(self._pal, "IconPickerColorPanel"))
    ...
    def _colors_panel(self):
        ...
        panel.setStyleSheet(panel_qss(self._pal, "IconPickerColorPanel"))
    ...
    def _style_color_option_button(self, button, color):
        ...
        button.setStyleSheet(color_chip_qss(self._pal, color))
    ...
    def _icons_tab(self):
        ...
        def add_group(title, items, filter_text=""):
            ...
            if not visible:
                return
            label = QLabel(title)
            label.setStyleSheet(section_title_qss(self._pal))
    ...
    def _upload_tab(self):
        ...
        for label, extensions in (("Upload SVG icon", "*.svg"), ("Upload PNG image", "*.png")):
            ...
            btn.setStyleSheet(f"QPushButton {{ background: {self._pal['inset']}; color: {self._fg}; border: none; border-radius: 14px; text-align: left; padding-left: 16px; }} QPushButton:hover {{ background: {self._pal['inset_hover']}; }}")
    ...
    def _cell_style(self, selected):
        ...
        return cell_qss(self._pal, selected, icon_color)
    ...
    def _icon_cell(self, item, image_mode=False):
        ...
        if item.get("is_add_button"):
            ...
            cell.setFixedSize(CELL_SIZE, CELL_SIZE)
            cell.setCursor(Qt.CursorShape.PointingHandCursor)
            cell.setStyleSheet(cell_qss(self._pal, False, icon_color))
            layout = QVBoxLayout(cell)
            layout.setContentsMargins(6, 6, 6, 6)
            preview = QLabel()
            preview.setAlignment(Qt.AlignmentFlag.AlignCenter)
