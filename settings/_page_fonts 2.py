...
class FontSizeSpinBox(QSpinBox):
    """Commits the typed size on Enter without letting the key reach the dialog."""
...
class PageFontsMixin:
    ...
    def _create_font_selector_group(self, title, config_key):
        ...
        size_pill = QWidget()
        size_pill.setObjectName("fontSizePill")
        size_pill.setAttribute(Qt.WidgetAttribute.WA_Hover, True)
        size_pill.setFixedHeight(44)
        size_pill.setMinimumWidth(102)
        size_pill.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        size_pill_layout = QHBoxLayout(size_pill)
        size_pill_layout.setContentsMargins(16, 0, 16, 0)
        size_pill_layout.setSpacing(2)

        size_spinbox = FontSizeSpinBox()
        size_spinbox.setObjectName("fontSizeSpinBox")
        size_spinbox.setRange(8, 72)
        size_spinbox.setFrame(False)
        size_spinbox.setButtonSymbols(QAbstractSpinBox.ButtonSymbols.NoButtons)
        ...
        unit_label.setObjectName("fontSizeUnitLabel")

        size_pill_layout.addWidget(size_spinbox)
        size_pill_layout.addWidget(unit_label)
        ...
        control_layout.addWidget(size_pill, 0, 1)
        ...
        def sync_font_controls_width():
            ...
            right_width = max(size_pill.minimumWidth(), add_button.sizeHint().width())
    ...
    def _update_font_preview(self, config_key):
        ...
        def resize_preview_labels():
            ...
            body_width = body_label.width()
            if preview is not None and preview.layout() is not None:
                margins = preview.layout().contentsMargins()
                body_width = max(body_width, preview.width() - margins.left() - margins.right())
            if body_width <= 1:
                # Layout not settled yet; the queued pass measures the real width.
                return
