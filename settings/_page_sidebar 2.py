...
class PageSidebarMixin:
    ...
    def _create_sidebar_section_toggle(self, title, content_widget):
        ...
        # Not a real submit action: Enter pressed anywhere else in the dialog
        # (e.g. the grid Columns/Rows spinboxes) must not trigger this as the
        # implicit default button and collapse the section.
    ...
    def _draw_sidebar_preview_actions(self, painter, rect, mode, add_button_height=None):
        ...
        # List mode (default): one labelled row per action. "Add" is drawn as a
        # dashed call-to-action bar only while that option is enabled; otherwise
        # it is an ordinary row like the rest.
        ...
        for action_id in ids:
            if action_id == "add" and self._action_add_dashed_enabled():
    ...
    def _save_sidebar_settings(self):
        ...
        # Dashed outline on the "Add" button (Action Button Customization card)
        if hasattr(self, "action_add_dashed_toggle"):
            self.current_config["sidebarAddDashed"] = self.action_add_dashed_toggle.isChecked()
        ...
        if hasattr(self, "sidebar_bg_color_only_toggle"):
            ...
            hide_profile_toggle = getattr(self, "sidebar_bg_hide_profile_toggle", None)
            if hide_profile_toggle is not None:
                mw.col.conf["modern_menu_hide_profile_bar"] = bool(hide_profile_toggle.isChecked())
