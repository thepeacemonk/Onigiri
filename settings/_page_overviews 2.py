...
from ._page_reviewer import _ReviewerStickyFilter
...
class PageOverviewsMixin:
    def create_overviews_page(self):
        ...
        self._add_navigation_buttons(page, page_scroll, sections)
        self._install_overview_style_sticky_header(page_scroll)

        return page

    def _install_overview_style_sticky_header(self, scroll):
        """Pin the Overview Style cap (top outline + header + live preview) to
        the top of the page while the options below it scroll. Mirrors
        _install_reviewer_sticky_header in _page_reviewer.py."""
        cap = getattr(self, "_overview_style_sticky_top", None)
        outer = getattr(self, "_overview_style_sticky_outer", None)
        if cap is None or outer is None or scroll is None:
            return
        self._overview_style_sticky_scroll = scroll
        self._overview_style_sticky_spacer = None
        self._overview_style_sticky_installed = False

        filt = _ReviewerStickyFilter(self._reposition_overview_style_sticky, self)
        self._overview_style_sticky_filter = filt
        scroll.viewport().installEventFilter(filt)
        cap.installEventFilter(filt)

        vbar = scroll.verticalScrollBar()
        vbar.valueChanged.connect(lambda _=None: self._reposition_overview_style_sticky())
        vbar.rangeChanged.connect(lambda *_: self._reposition_overview_style_sticky())
        QTimer.singleShot(0, self._reposition_overview_style_sticky)

    def _overview_style_cap_height(self, cap, width, fallback):
        try:
    ...
    def _reposition_overview_style_sticky(self):
        if getattr(self, "_overview_style_sticky_busy", False):
            return
        self._overview_style_sticky_busy = True
        try:
            scroll = getattr(self, "_overview_style_sticky_scroll", None)
            cap = getattr(self, "_overview_style_sticky_top", None)
            designer = getattr(self, "overview_style_designer", None)
            ...
            if not getattr(self, "_overview_style_sticky_installed", False):
                ...
                if idx < 0:
                    return
                spacer = QWidget()
                spacer.setObjectName("overviewStyleStickySpacer")
                ...
                # Opaque backing rect, square corners, sits directly behind the cap so
                # scrolled body content can never peek through the cap's rounded top
                # corners.
                shade = QWidget(viewport)
                shade.setObjectName("overviewStyleStickyShade")
                shade.setStyleSheet(
                    f"QWidget#overviewStyleStickyShade {{ background-color: {self._overview_style_sticky_panel_bg}; border: none; }}"
                )
                shade.show()
                self._overview_style_sticky_shade = shade

                cap.raise_()
                self._overview_style_sticky_spacer = spacer
                self._overview_style_sticky_installed = True

            spacer = getattr(self, "_overview_style_sticky_spacer", None)
            ...
            if width <= 1:
                width = cap.width()
            height = self._overview_style_cap_height(cap, width, spacer.height())
            ...
            shade = getattr(self, "_overview_style_sticky_shade", None)
            if shade is not None and not (sip is not None and sip.isdeleted(shade)):
                shade.setGeometry(x, target_y, width, height)
                shade.raise_()
            ...
            self._overview_style_sticky_busy = False
    ...
    def _create_overview_style_group(self):
        # Same pinned-cap / scrolling-body split as the Reviewer Buttons panel
        # (see _install_reviewer_sticky_header): the preview stays attached to
        # the top of the page while the color/effect controls below it scroll.
        designer = QFrame()
        designer.setObjectName("overviewStyleDesigner")
        designer.setStyleSheet(
            "QFrame#overviewStyleDesigner { background: transparent; border: none; }"
        )
        container_layout = QVBoxLayout(designer)
        ...
        sticky_cap = QFrame()
        sticky_cap.setObjectName("overviewStyleStickyCap")
        sticky_cap.setStyleSheet(
            f"QFrame#overviewStyleStickyCap {{ background-color: {_sticky_panel_bg}; "
        ...
        outer = QVBoxLayout(sticky_cap)
        outer.setContentsMargins(18, 18, 18, 14)
        outer.setSpacing(18)

        body = QFrame()
        body.setObjectName("overviewStyleBody")
        body.setStyleSheet(
            f"QFrame#overviewStyleBody {{ background-color: {_sticky_panel_bg}; "
        ...
        # Refs used by _install_overview_style_sticky_header (called from
        # create_overviews_page). The whole cap (top outline + header + preview)
        # is the pinned element; it is lifted out of `container_layout` into the
        # scroll viewport.
        self._overview_style_sticky_top = sticky_cap
        self._overview_style_sticky_outer = container_layout
        self._overview_style_sticky_installed = False
        self._overview_style_sticky_panel_bg = _sticky_panel_bg
        self.overview_style_designer = designer
        ...
        self.overview_style_preview = BackgroundPreviewLabel(aspect_ratio=2.69, minimum_preview_height=376)
        ...
        # Same segmented-pill layout as "Grid Alignment" in the Main Menu's
        # Widget Grid settings (_create_organize_segmented_control).
        ...
        style_container = self._create_organize_segmented_control(
            [("pro", "Pro"), ("mini", "Mini")],
            self.overview_style_mode_group, current_style, "overview_style_design",
            fill_width=True, segment_height=26, min_button_width=64,
        )
        style_container.setFixedHeight(34)
        for button in self.overview_style_mode_group.buttons():
            if button.property("overview_style_design") == "mini":
                self.overview_mini_radio = button
            else:
                self.overview_pro_radio = button

        # All toggles live together on the left, under Design, alongside the
        # text fields above them.
        ...
        option_controls_layout.addWidget(self._create_main_bg_toggle_row("Sync with Widget Color and Effect", self.overview_style_sync_toggle))
        option_controls_layout.addWidget(self._create_main_bg_toggle_row("Dashed stroke", self.overview_study_button_dashed_toggle))
        option_controls_layout.addWidget(self._create_main_bg_toggle_row("Animated (hover effect)", self.overview_study_button_animated_toggle))
        ...
        body_layout.addWidget(preview_controls_pair)
        body_layout.addWidget(colors_categories)
    ...
    def _update_overview_style_controls(self):
        ...
        if hasattr(self, "overview_style_preview_mode_toggle"):
            ...
            self.overview_style_preview_mode_toggle.setToolTip(
                "Following Widget Color and Effect's light/dark preview while synced." if synced else ""
            )
        self.overview_effects_wrapper.setEnabled(not synced)
        lock_hint = "Disable Sync with Widget Color and Effect to edit this control."
        ...
        if hasattr(self, "overview_style_surface_header"):
            self.overview_style_surface_header.setToolTip(
                "Box Surface colors are read-only while Sync with Widget Color and Effect is active."
                if synced else ""
            )
        ...
        for key in ("box_bg", "box_border"):
            for mode in ("light", "dark"):
                ...
                if button:
                    ...
                    button.setToolTip(
                        "Disable Sync with Widget Color and Effect to edit this color."
                        if synced else ""
                    )
                if line_edit:
    ...
    def _draw_overview_style_sample(self, painter, rect, mode, background_pixmap, draw_profile=True):
        ...
        show_profile = (
            hasattr(self, "show_overview_profile_bar_checkbox")
            and self.show_overview_profile_bar_checkbox.isChecked()
            and profile_type == "bar"
        )
        # Every fixed pixel measurement below is tied to `scale` so the whole
        # sample card (text, gaps, bubbles, button) shrinks/grows together with
        # the preview's actual height instead of overflowing it. 470 is the
        # preview's historical reference height (scale == 1 there).
        scale = max(0.6, min(1.3, rect.height() / 470.0))
        main_font_size = int(mw.col.conf.get("onigiri_font_size_main", 14) or 14)
        title_pixel_size = int((20 if is_mini else max(18, main_font_size * 2)) * scale)
        ...
        title_height = int(30 * scale) if is_mini else max(int(32 * scale), title_pixel_size + 8)
        title_gap = max(2, int((4 if is_mini else 6) * scale))
        button_gap = max(4, int((8 if is_mini else 12) * scale))
        button_height = max(20, int((30 if is_mini else 38) * scale))
        profile_bar_base = (32 if is_mini else 38) * scale
        profile_overlap = (profile_bar_base * (0.50 if is_mini else 0.56)) if show_profile else 0
        visible_profile_height = (profile_bar_base - profile_overlap) if show_profile else 0

        path = QPainterPath()
        if is_mini:
            box_width = min(int(rect.width() * 0.48), 300)
            box_height = min(int(rect.height() * 0.26), 120) if show_profile else min(int(rect.height() * 0.23), 105)
        else:
            box_width = min(int(rect.width() * 0.62), 360)
            box_height = min(int(rect.height() * 0.40), 185) if show_profile else min(int(rect.height() * 0.36), 165)
        ...
        # Center the whole title+box+button block vertically in the container —
        # same rule for Pro and Mini, so neither ever pins to a fixed top offset.
        group_top = rect.y() + max(4 * scale, (rect.height() - total_height) / 2)
        box_y = group_top + title_height + title_gap + visible_profile_height
        max_box_bottom = rect.bottom() - button_gap - button_height - 10 * scale
        if box_y + box_height > max_box_bottom:
            box_height = max(int(120 * scale) if is_mini else int(170 * scale), int(max_box_bottom - box_y))
            total_height = title_height + title_gap + visible_profile_height + box_height + button_gap + button_height
            group_top = rect.y() + max(4 * scale, (rect.height() - total_height) / 2)
            box_y = group_top + title_height + title_gap + visible_profile_height
        ...
        body_font = self._box_effect_preview_font("main", max(9, int((19 if not is_mini else 15) * scale)))
        painter.setFont(body_font)

        if is_mini:
            stats_y = box_rect.y() + 27 * scale
            row_height = 30 * scale
            row_positions = [stats_y, stats_y + row_height, stats_y + row_height * 2]
        else:
            row_gap = max(48 * scale, min(62 * scale, box_rect.height() * 0.24))
        ...
        line_left = box_rect.x() + 24 * scale
        line_right = box_rect.right() - 24 * scale
        bubble_center_offset = (9 if is_mini else 11) * scale
        ...
        def draw_bubble(y, text_label, count, bg_key, fg_key):
            ...
            bubble_rect = QRectF(
                box_rect.x() + box_rect.width() - (100 if not is_mini else 84) * scale,
                y - (4 if not is_mini else 3) * scale,
                (78 if not is_mini else 64) * scale,
                (30 if not is_mini else 24) * scale,
            )
            label_rect = QRectF(
                box_rect.x() + 24 * scale,
                bubble_rect.y(),
                bubble_rect.x() - box_rect.x() - 44 * scale,
                bubble_rect.height(),
            )
            painter.setPen(QColor(self._box_effect_text_color(mode)))
        ...
        button_width = box_rect.width() - 40 * scale if is_mini else box_rect.width()
        button_x = box_rect.x() + 20 * scale if is_mini else box_rect.x()
        ...
        painter.setPen(QColor(self._box_effect_text_color(mode)))
        button_font = self._box_effect_preview_font("main", max(9, int((16 if not is_mini else 13) * scale)))
        ...
        painter.drawText(
            ...
            metrics.elidedText(study_text, Qt.TextElideMode.ElideRight, int(button_rect.width() - 20 * scale)),
        )
