...
class OnigiriDraggableItem(DraggableItem):
    ...
    def _open_stats_title_editor(self):
        """Opens the Stats Title editor and stages the result on the settings
        dialog, so it lands in col.conf with everything else on Save."""
        settings_dialog = self.grid_zone.main_editor.settings_dialog
        dialog = StatsTitleDialog(
            settings_dialog.stats_title_input.text(),
            getattr(settings_dialog, "selected_stats_title_font_key", STATS_TITLE_SYNC_KEY),
            settings_dialog.addon_path,
            settings_dialog,
        )

        def on_accepted(text, font_key):
            settings_dialog.stats_title_input.setText(clamp_stats_title(text))
            settings_dialog.selected_stats_title_font_key = font_key or STATS_TITLE_SYNC_KEY
    ...
    def contextMenuEvent(self, event):
        ...
        is_stats_title = self.widget_id == 'stats_title'
        ...
        for i in range(1, max_cols + 1):
            ...
            if self.widget_id == 'deck_stats':
                if i > 2: continue
            if self.widget_id == 'onigimon':
                if i > 2: continue # Onigimon: 1 col (portrait card) or 2 cols (wide card)
        ...
        if is_heatmap:
            ...
            max_rows = 4  # Onigimon: 1 row is the compact companion-only view, 2-4 rows the full card
            ...
            max_rows = 1  # Stats Title: a single line of text
        ...
        # --- Edit Title Action ---
        if is_stats_title:
            edit_title_button = QPushButton(tr("edit_title", "Edit Title"))
            edit_title_button.setObjectName("MenuButton")
            # The menu is a WA_DeleteOnClose popup, so let it finish tearing
            # down before exec()ing a modal dialog from one of its buttons.
            edit_title_button.clicked.connect(
                lambda: (custom_menu.close(), QTimer.singleShot(0, self._open_stats_title_editor))
            )
            custom_menu.content_layout.addWidget(edit_title_button)
            custom_menu.add_separator()
...
class MainMenuLayoutEditor(QWidget):
    ...
    def get_layout_config(self):
        ...
        archive_config = self.archive_zone.get_archive_config()
        # Raw parts only: the caller (SettingsDialog.apply_changes) is what maps
        # these into onigiriWidgetLayout / externalWidgetLayout.
        return {
            "grid": grid_config, 
...
class UnifiedLayoutEditor(QWidget):
        ...
        for widget_id, config in onigiri_grid_config.items():
            if item := self.all_onigiri_items.get(widget_id):
                ...
                if widget_id == "prep_station":
                    ...
                    item.row_span = 2
                    item.col_span = max(1, min(4, item.col_span))
                if widget_id == "stats_title":
                    # The title is a single line, so it is always exactly 1 row tall
                    item.row_span = 1
                    item.col_span = max(1, min(4, item.col_span))
                if self.grid_zone.place_item(item, config.get("pos", 0), silent=True):
    ...
    def _archive_onigiri_item(self, item):
        ...
        if item.widget_id == "heatmap":
            ...
            item.row_span, item.col_span = 2, 2
        elif item.widget_id == "stats_title":
            item.row_span, item.col_span = 1, 4
        else:
            item.row_span, item.col_span = 1, 1
        ...
        for pos, shelf in self.grid_zone.shelves.items():
            ...
            if widget and widget not in processed_widgets:
                ...
                if isinstance(widget, SettingsDialog.OnigiriDraggableItem):
                    ...
                    elif widget.widget_id == "stats_title":
                        row_span = 1
                        col_span = max(1, min(4, col_span))
                    widget_config = {
    ...
    def reset_widget_names(self):
        ...
        if changes_made:
            show_settings_toast(self.settings_dialog, tr("widget_names_reset_toast", "Widget names reset to defaults"))
        else:
            show_settings_toast(self.settings_dialog, tr("widget_names_already_default_toast", "Widget names are already at defaults"))
