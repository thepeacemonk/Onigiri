...
from . import config
from .translations import tr as onigiri_tr
...
class RestaurantLevelDialog(QDialog):
    def __init__(self, parent):
        ...
        self.setWindowTitle(onigiri_tr("restaurant_level", "Nook Level"))
    ...
    <section class="profile-restaurant-level" data-level="{level}" {style_attr}>
        <header class="prl-header">
            <div class="prl-title-group">
                <span class="prl-title">{onigiri_tr("restaurant_level", "Nook Level")}</span>
...
def generate_icon_css(addon_package, conf):
    ...
    if not closed_icon_url:
        closed_icon_url = get_data_uri(os.path.join(addon_dir, "system_files", "system_icons", "chevron.svg"))

    open_icon_url = ""
    if open_icon_file:
        open_icon_url = get_data_uri(os.path.join(addon_dir, "user_files", "icons", open_icon_file))
    if not open_icon_url:
        open_icon_url = get_data_uri(os.path.join(addon_dir, "system_files", "system_icons", "chevron.svg"))
