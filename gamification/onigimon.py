...
class OnigimonManager:
    ...
    def use_item(self, item_key: str) -> Optional[str]:
        ...
        if item_key == "pokeballs":
            return f"{self.companion_display_name(companion)} is saving Pok├®balls for future Pok├®mon."
...
def render_widget_html() -> str:
    ...
    pokeball_icon = _item_asset_url(ITEMS["pokeballs"])

    if status == "disabled":
        body = "<p>Enable Onigimon in Gamification Settings.</p>"
    elif status == "missing":
        body = "<p>Install Ankimon to choose a Pok├®mon companion.</p>"
    elif status == "starter_needed":
        body = "<p>Finish choosing your first Ankimon Pok├®mon, then come back to Onigimon.</p>"
    elif status == "no_collection":
        body = "<p>Your Ankimon starter is ready. Add Pok├®mon to your Collection PC to choose a companion.</p>"
    elif not companion:
        body = "<p>Choose a Pok├®mon from Ankimon's Collection PC in Gamification Settings.</p>"
    else:
        name = escape(manager.companion_display_name(companion))
        sprite = companion.get("sprite_url") or ""
        placeholder = f'<img class="onigimon-placeholder" src="{escape(pokeball_icon)}" alt="">' if pokeball_icon else ""
        img = f'<img src="{escape(sprite)}" alt="">' if sprite else placeholder
        body = f"""
            <div class="onigimon-main">
                <div class="onigimon-sprite">{img}</div>
                <div class="onigimon-info">
                    <strong>{name}</strong>
                    <span>Lv. {int(companion.get("level") or 1)} ┬À Bond {int(companion.get("bond_level") or 1)}</span>
                </div>
            </div>
...
def _ball_icon_html(icon_url: str) -> str:
    if icon_url:
        return f'<img class="onigimon-ball-icon" src="{escape(icon_url)}" alt="">'
    return '<span class="onigimon-ball-icon"></span>'


def widget_css() -> str:
    return """
    <style id="onigimon-widget-styles">
        .onigimon-widget {
            display: flex;
        ...
        .night .onigimon-widget,
        .night-mode .onigimon-widget,
        .nightMode .onigimon-widget {
        ...
        .onigimon-ball-btn {
            ...
            border: 1px solid var(--border, #e0e0e0);
            border-radius: 999px;
            background: color-mix(in srgb, var(--fg, #222222) 5%, transparent);
            padding: 0;
            cursor: pointer;
        }
        ...
        span.onigimon-ball-icon {
            border-radius: 50%;
            background: var(--fg-subtle, #757575);
        }
        ...
        .onigimon-meter > div {
            ...
            border-radius: 999px;
            overflow: hidden;
            background: color-mix(in srgb, var(--fg, #222222) 10%, transparent);
        }
        ...
        .night .onigimon-care-dialog,
        .night-mode .onigimon-care-dialog,
        .nightMode .onigimon-care-dialog {
            background: var(--canvas, #202020);
        }
        ...
        .onigimon-category-panel {
            ...
            border: 1px solid var(--border, #e0e0e0);
            border-radius: 10px;
            background: color-mix(in srgb, var(--fg, #222222) 4%, transparent);
        }
...
def _care_modal_html(payload: Dict[str, Any], companion: Optional[Dict[str, Any]]) -> str:
    ...
    close_icon = _addon_asset_url("system_files/system_icons/cancel.svg")
    ...
    categories = (
        ...
        ("pokeballs", "Pok├®balls", "pokeballs", ("pokeballs",), "none"),
    )
    category_bits = "".join(
...
def _inventory_choice_html(item_key: str, action: str, count: int, companion: Dict[str, Any]) -> str:
    ...
    if action == "none":
        ...
        <div class="onigimon-inventory-choice is-passive" {_item_color_style(item_key)} title="{escape(hint)}">
            {_item_icon(item_key)}
            <span>{escape(label)}</span>
            <small>{escape(hint)} ┬À {count}</small>
        </div>
        """
    return f"""
